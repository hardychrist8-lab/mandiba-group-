...
/* Wrapper : Link pour Assurances, Button pour Transport (ouvre catalogue) */
...
export function Activities() {
  const openCatalogue = useCatalogue((s) => s.openCatalogue);

  const cardClassName =
    "group relative block h-full p-8 lg:p-10 rounded-2xl bg-card border border-border overflow-hidden hover:shadow-mandiba-lg transition-all duration-500 hover:-translate-y-1";
      ...
      <div className="container-mandiba relative">
        ...
        <div className="grid md:grid-cols-2 gap-6 lg:gap-8 max-w-5xl mx-auto">
          {activities.map((activity, i) => {
            const isTransport = activity.id === "transport";
            return (
                ...
                >
                  ...
                  <div className="relative">
                    ...
                    <div className="flex items-center gap-2 text-primary font-semibold text-sm group-hover:text-accent group-hover:gap-3 transition-all">
                      {isTransport ? "Voir le catalogue" : "En savoir plus"}
